...
file_path <- "TrumpTweets\\realdonaldtrump.csv"
...
plots_dir <- "RealTrumpTweetsPlots"
...
if ("year_month" %in% colnames(tweets_df) && "datetime" %in% colnames(tweets_df)) {
  ...
  if (nrow(tweets_per_month) > 0) {
    ...
    print(p1)
    ggsave(file.path(plots_dir, "realdonaldtrump_tweets_per_month.png"), plot = p1, width = 10, height = 6, dpi = 300)
    cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_tweets_per_month.png"), "\n")
  } else {
...
p2 <- ggplot(tweets_df, aes(x = tweet_length)) +
  ...
  theme_minimal()
print(p2)
ggsave(file.path(plots_dir, "realdonaldtrump_tweet_length_distribution.png"), plot = p2, width = 8, height = 6, dpi = 300)
cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_tweet_length_distribution.png"), "\n")
...
if (nrow(word_counts) > 0) {
  ...
  saveWidget(wordcloud2_fig, file.path(plots_dir, "realdonaldtrump_wordcloud.html"), selfcontained = FALSE, title = "Trump Word Cloud")
  cat("Word cloud saved as", file.path(plots_dir, "realdonaldtrump_wordcloud.html"), "\n")
  tryCatch({
    webshot2::webshot(file.path(plots_dir, "realdonaldtrump_wordcloud.html"), file.path(plots_dir, "realdonaldtrump_wordcloud.png"), delay = 2, vwidth = 1000, vheight = 800)
    cat("Word cloud saved as", file.path(plots_dir, "realdonaldtrump_wordcloud.png"), "\n")
  }, error = function(e) {
...
if (nrow(word_counts) > 0) {
  p4 <- ggplot(head(word_counts, 20), aes(x = reorder(word, n), y = n)) +
    ...
    theme_minimal()
  print(p4)
  ggsave(file.path(plots_dir, "realdonaldtrump_top20_words.png"), plot = p4, width = 8, height = 7, dpi = 300)
  cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_top20_words.png"), "\n")
} else {
...
if (nrow(hashtags_df) > 0) {
  p5 <- ggplot(head(hashtags_df, 20), aes(x = reorder(hashtags, n), y = n)) +
    ...
    theme_minimal()
  print(p5)
  ggsave(file.path(plots_dir, "realdonaldtrump_top20_hashtags.png"), plot = p5, width = 8, height = 7, dpi = 300)
  cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_top20_hashtags.png"), "\n")
} else {
...
if (nrow(mentions_df) > 0) {
  p6 <- ggplot(head(mentions_df, 20), aes(x = reorder(mentions, n), y = n)) +
    ...
    theme_minimal()
  print(p6)
  ggsave(file.path(plots_dir, "realdonaldtrump_top20_mentions.png"), plot = p6, width = 8, height = 7, dpi = 300)
  cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_top20_mentions.png"), "\n")
} else {
...
if (nrow(words_cleaned) > 0) {
  ...
  if (nrow(bing_sentiments) > 0) {
    p7 <- ggplot(bing_sentiments, aes(x = sentiment, y = n, fill = sentiment)) +
      ...
      theme_minimal()
    print(p7)
    ggsave(file.path(plots_dir, "realdonaldtrump_sentiment_bing.png"), plot = p7, width = 7, height = 5, dpi = 300)
    cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_sentiment_bing.png"), "\n")
  } else {
...
if ("datetime" %in% colnames(tweets_df) && "id" %in% colnames(words_cleaned)) {
  ...
  if (nrow(sentiment_with_dates) > 0) {
    p8 <- ggplot(sentiment_with_dates, aes(x = year_month, y = n, fill = sentiment)) +
      ...
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    print(p8)
    ggsave(file.path(plots_dir, "realdonaldtrump_sentiment_over_time.png"), plot = p8, width = 10, height = 6, dpi = 300)
    cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_sentiment_over_time.png"), "\n")
  } else {
...
if ("day_of_week" %in% colnames(tweets_df) && "datetime" %in% colnames(tweets_df)) {
  ...
  if(nrow(tweets_by_dow) > 0) {
    p9 <- ggplot(tweets_by_dow, aes(x = day_of_week, y = n, fill = day_of_week)) +
      ...
      theme_minimal()
    print(p9)
    ggsave(file.path(plots_dir, "realdonaldtrump_tweets_by_dayofweek.png"), plot = p9, width = 8, height = 5, dpi = 300)
    cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_tweets_by_dayofweek.png"), "\n")
  } else {
...
if ("hour_of_day" %in% colnames(tweets_df) && "datetime" %in% colnames(tweets_df)) {
  ...
  if(nrow(tweets_by_hour) > 0) {
    p10 <- ggplot(tweets_by_hour, aes(x = hour_of_day, y = n)) +
      ...
      theme_minimal()
    print(p10)
    ggsave(file.path(plots_dir, "realdonaldtrump_tweets_by_hour.png"), plot = p10, width = 9, height = 5, dpi = 300)
    cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_tweets_by_hour.png"), "\n")
  } else {
...
if (nrow(bigrams) > 0) {
  ...
  if (nrow(bigram_counts) > 0) {
    p11 <- ggplot(head(bigram_counts, 20), aes(x = reorder(bigram, n), y = n)) +
      ...
      theme_minimal()
    print(p11)
    ggsave(file.path(plots_dir, "realdonaldtrump_top20_bigrams.png"), plot = p11, width = 8, height = 7, dpi = 300)
    cat("Plot saved:", file.path(plots_dir, "realdonaldtrump_top20_bigrams.png"), "\n")
  } else {
